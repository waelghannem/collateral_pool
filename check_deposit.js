...
function showListTransactions(deposit_address,amount) {

    return new Promise(function (resolve, reject) {
        exec(`sh hmn_showListTransactions.sh hostmasternode-cli`, (err, stdout, stderr) => {
            if (err) {
                reject(err)
            } else if (stderr) {
                reject(stderr)
                ...
                if (stdout) {
                    ...
                    for (transactions of res) {
                        if (transactions.category == "receive") {
                            ...
                            console.log("condition address = ",(transactions.address.trim() == deposit_address.trim()))
                            console.log("condition amount = ",(transactions.amount == amount))
                            
                            if (transactions.address.trim() == deposit_address.trim() && transactions.amount == amount) {
                            ...
                            for (credentials of pool_params.COINS_CREDENTIALS) { // 1 fois
                                if (credentials.name == request._id) {
                                    try {
                                        ...
                                        tab = await showListTransactions(addresspercoin.deposit_address,addresspercoin.allstatus[addresspercoin.allstatus.length - 1].amount)
                                        console.log("listransactions is : ",tab);
                                        if (tab) {
